	...
	namespace RandomEffects
	{
		namespace Stratified
		{
			...
			void BlockSection::AddIdentityModel(const ImmutableVector<double> & theta)
			{
				...
				this->covariance_model.Reset(new(bl) T(this->variables.Size(), this->factor.NumberOfLevels(), theta));
			}

			void BlockSection::AddMultivariateIdentityModel(const ImmutableVector<double> & theta)
			{
				if (this->covariance_model.IsNull() == false)
					throw Exceptions::MultipleCovarianceModelsException();

				// Add covariance model
				typedef GlmmGS::RandomEffects::Stratified::CovarianceModels::MultivariateIdentityModel T;
				this->covariance_model = Pointer<T>(new(bl) T(this->variables.Size(), this->factor.NumberOfLevels(), theta));
			}
			...
			void BlockSection::AddPrecisionModel(const ImmutableMatrix<double> & R, const ImmutableVector<double> & theta)
			{
				...
				this->covariance_model = Pointer<T>(new(bl) T(this->variables.Size(), R, theta));
			}

			void BlockSection::AddSparsePrecisionModel(const LDL::SparseMatrix<double> & R, const ImmutableVector<double> & theta)
			{
				if (this->covariance_model.IsNull() == false)
					throw Exceptions::MultipleCovarianceModelsException();
			...
			void BlockSection::EndStratifiedBlock()
			{
				...
				this->data->random_effects.Add(Pointer<T>(new(bl) T(this->variables.ToVector(), this->factor, this->covariance_model)));
			}
		}
	}
}
